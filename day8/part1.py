...
permuted_digit_code_mappings = {tuple(sorted(tuple(code) for code in digit_codes[:, mapping])): mapping for mapping in all_mappings}
...
def translate(observed_codes, target_codes):
    observed_codes = tuple(sorted(tuple(alpha_to_bin_array(c)) for c in observed_codes.split()))
...
def main(lines):
    ...
    for line in lines:
        ...
        translations.append(translate(all_codes, target_codes))

    translations = np.array(translations).flatten()
    return np.isin(translations, [1, 4, 7, 8]).sum()
